...
[in R version 4.1.0](https://blog.r-project.org/2020/07/15/new-features-in-the-r-graphics-engine/).
...
In this case, we use a "clear" operator, which means that,
...
grid.group(rectGrob(width=.4, height=.2),
           "clear",
...
"clear" operator so that the rectangle creates a hole in the circle), 
in a viewport
...
grob <- groupGrob(rectGrob(width=.4, height=.2),
                  "clear",
                  circleGrob(r=.4, gp=gpar(col=NA, fill=4)))
...
and luminance masks only work on the `pdf()` device.
