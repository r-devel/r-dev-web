...
invisible(file.copy(gcc_warn, "/vols/ftp/pub/bdr/gcc", overwrite =  TRUE,
                    copy.date = TRUE))

ff <- list.files("/vols/ftp/pub/bdr/gcc", pattern = patt)

old <- setdiff(ff, gcc_warn)
unlink(file.path("/vols/ftp/pub/bdr/gcc", old))

ff <- list.files("/vols/ftp/pub/bdr/gcc", pattern = patt)
invisible(file.copy(ff, "/vols/ftp/pub/bdr/gcc", overwrite =  TRUE,
                    copy.date = TRUE))


Sys.setlocale("LC_COLLATE", "C") -> junk

files <- list.files("/vols/ftp/pub/bdr/gcc", pattern = patt, full.names = TRUE)
...
write.csv(DF, "~/Rlogs/memtests/gcc.csv",
