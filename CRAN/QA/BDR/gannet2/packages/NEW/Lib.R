...
## rest is mailx_CRAN_package_problems.R
...
record_CRAN_package_issues <-
function(packages, from = Sys.getenv("EMAIL"), before, verbose = TRUE)
{       
    headers <-
        paste0("X-CRAN-Issue-Info: ",
               CRAN_issue_info(NULL, before))
        
    body <- c("Team: recording deadline.")
