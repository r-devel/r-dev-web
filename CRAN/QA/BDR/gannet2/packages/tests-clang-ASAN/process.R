...
for(f in files) {
	...
	f2 <- dirname(f)
        dir.create(file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff),
                             showWarnings = FALSE, recursive = TRUE)
        file.copy(f, file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff, "00check.log"),
...
for(f in files) {
	...
	cat(".")
        ff <- sub("[.]Rcheck/.*", "", f)
        dir.create(d <- file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff, "tests"),
                             showWarnings = FALSE, recursive = TRUE)
        f2 <- sub(".*[.]Rcheck/", "", f)
        file.copy(f,file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff, f2), overwrite=TRUE, copy.date = TRUE)
...
for(f in files) {
    ...
    if(length(ll)) {
        ...
        dir.create(file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff),
                             showWarnings = FALSE, recursive = TRUE)
        f2 <- sub(".*[.]Rcheck/", "", f)
        file.copy(f,file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff, f2),
                  overwrite = TRUE, copy.date = TRUE)
    }
}
cat("\n")
...
##    ll2 <- ll2[!all(grepl("omp_in_parallel", ll2, useBytes = TRUE))]
    ...
    if(length(ll)) {
        cat(".")
        ff <- sub("[.]Rcheck/.*", "", f)
        dir.create(d <- file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff),
                                  showWarnings = FALSE, recursive = TRUE)
        file.copy(f, file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff),
			      overwrite = TRUE, copy.date = TRUE)
        file.copy(sub("00install.out", "00check.log", f),
                  file.path("/vols/ftp/pub/bdr/memtests/clang-ASAN", ff),
			      overwrite = TRUE, copy.date = TRUE)
        Sys.setFileTime(d, file.info(dirname(f))$mtime)
    }
}
...
for(d in list.dirs('/vols/ftp/pub/bdr/memtests/clang-ASAN', TRUE, FALSE)) {
