files <- list.files("/data/ftp/pub/bdr/clang19", pattern = "[.](out|log)$", full.names = TRUE)
...
DF <- data.frame(Package = Package, Version = Versions,
                 ...
                 href = paste0("https://www.stats.ox.ac.uk/pub/bdr/clang19/", basename(files)),
...
write.csv(DF, "/data/gannet/Rlogs/memtests/clang19.csv", row.names = FALSE, quote = FALSE)
