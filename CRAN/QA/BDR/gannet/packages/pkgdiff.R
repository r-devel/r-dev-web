diff1 <- function(from, to)
{
	...
	txt <- grep("^\\* (used|using) (C|Fortran)", txt, invert = TRUE, value = TRUE, useBytes = TRUE)
	txt <- grep("^\\* running under:", txt, invert = TRUE, value = TRUE, useBytes = TRUE)
	txt <- grep("^(\\* R was compiled| *gcc| *clang| *GNU Fortran| *Intel| *ifx)", txt, invert = TRUE, value = TRUE, useBytes = TRUE)
