...
RAII (resource utilization is initialization) is a feature/idiom sometimes
...
is possible using `R_UnwindProtect`, see [Writing R Extensions
6.12](https://cran.r-project.org/doc/manuals/r-devel/R-exts.html#Condition-handling-and-cleanup-code),
but requires some verbose coding/boiler-plate.  Rcpp uses this API.
...
any destructor run when the same function goes out of scope allocates, R GC
...
debugging these things is far from trivial, this case we were lucky that
`ASAN` caught it.
...
exceptions, one would have to handle those, instead, possibly using
statically allocated).  In case of an error this memory is permanently
leaked.  With C, one can use say `R_alloc` that is deallocated automatically
and also on a long jump (see [Writing R Extensions
