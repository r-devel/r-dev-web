...
for the sorting is not feasible.)
...
which in the light of the above is what should really be avoided!  One
will thus also need to change the `data()` code accordingly, presumably
temporarily to honor `_R_OPTIONS_STRINGS_AS_FACTORS_`, and eventually to
always use `as.is = TRUE`.
