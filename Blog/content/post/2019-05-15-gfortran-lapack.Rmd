...
GFortran, and with -fno-optimize-sibling-calls): the hidden length arguments
were last on the argument list, so with common calling conventions that
allow variadic arguments they did not cause trouble when present neither
when missing. The only situation when they are now required is the tail-call
optimizations with recent GFortran.
...
BLAS/LAPACK still works with GFortran.  After all, a Fortran compiler that
would not work with BLAS/LAPACK (abstracting from whose fault it is) would
be of little use.  For the time being, everyone should use
`-fno-optimize-sibling-calls` with GFortran version 7 and newer.  This is
already the default in R-Devel and R-Patched, but unfortunately it has to be
added explicitly when building released versions of R with GFortran version
7 and newer.

R packages written in a mix of C and Fortran should not depend on the
non-standardized interoperability behavior of the Fortran compilers (Writing
R Extensions discourages the use of strings, even of length one), except
when calling to BLAS/LAPACK.
...
standard).  Trying to fix this individually in packages or R using different
...
change after these incompatibilities are no longer harmless).
