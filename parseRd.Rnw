...
in all parts of an Rd file; details are given below.
...
alphabetic character must start a recognized macro, or a syntax error is signalled.
...
and may also appear within text, provided they balance.  The parser keeps
...
of the line.  These are ignored by the parser.
...
There are two types of comments in R-like mode.  The parser normally ignores
text starting with \verb!%!, and running to the end of the line (but see
the one exception below).

R-like comments start with \verb!#! and run to the end of the line, or a brace
...
There is no support for comments within verbatim text.
...
\item Within verbatim text, only braces need to be escaped.  Comments using
\verb!%! are not supported.
