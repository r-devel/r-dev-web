...
systems when the system is complete; currently no attempt is made 
...
In quoted strings in R-like code,
braces need not be escaped.  For example, \verb!\code{ "{" }! is legal.  
...
to be a markup macro if recognized).
...
\item Escapes are more regular now than they were, with the four
escapes \verb!\\!, \verb!\{!, \verb!\}!, and \verb!\%! recognized
in almost all contexts.  Even in verbatim
text, \verb!\{! will be interpreted as an escaped brace, so it will be
parsed as \verb!{!.  This will affect verbatim macros 
containing \LaTeX\ markup, e.g. \verb!\eqn{\{1,\dots,N-1\}}!, which
will now need to double two of the backslashes to
\verb!\eqn{\\{1,\dots,N-1\\}}! in order to pass them through.
(Doubling the backslash on \verb!\dots! is optional: \verb!\d!
is not an escape sequence in Rd files, so the backslash is left alone.)
...
(e.g. \verb!\code! or \verb!#ifdef!).  Zero argument markup which is followed by 
\verb!{}! will include the braces as part of the tag.
