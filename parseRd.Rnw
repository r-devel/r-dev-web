...
\abstract{
R 2.9.x introduced a parser for Rd
format help files.  When integrated into the build/install process in R 2.10.x,
it will allow easier processing,  
...
R 2.10.x will include some changes to the format, including some Sweave-like
...
function and its output.  Some additions to the specification have been made in R-devel
(to become R 2.12.x); these are noted earlier and 
...
\section{The \texttt{Sexpr} macro in R 2.10.0}
\label{sec:Sexpr}

R 2.10.0 will contain the new macros \verb!\Sexpr! and \verb!\RdOpts!.  These are
...
\section{New macros in R 2.12.0}
...
all take verbatim arguments.  See  Writing R Extensions for a discussion of their use.
