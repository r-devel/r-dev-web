...
and the structure returned from the new \verb!parse_Rd()! function.
}
...
2.8.0.  Finally, the last section describes the \verb!parse_Rd()!
...
where braces need not be escaped.  That is, \verb!\code{ "{" }! is legal.
...
macros is R-like, and that within \verb!\preformatted{}! macros is verbatim.
...
parseable by \verb!parse()!, but \verb!parse_Rd()! does not enforce this.
...
Rd parser will return R comments as part of the text of the code; the LaTeX-like
...
braces and R-like comments, but \verb!%! characters must be escaped 
...
is not accepted by the \verb!parse_Rd()! parser.
...
The \verb!parse_Rd()! function takes a text connection and produces a parsed version of
...
\begin{description}
\item[class] The object returned by \verb!parse_Rd()! is of class RdFile.
...
\item[srcref and srcfile] Objects may in the future have source references added; currently
this is incomplete.
\end{description}
