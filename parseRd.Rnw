...
Whitespace (blanks, tabs and formfeeds) at the start of text in LaTeX-like
sections and at the top level are returned as tokens.

Blank lines at the start of text in LaTeX-like sections and at the top level
are ignored.
...
There are two cases of special handling of braces.  In quoted strings in R-like code,
braces need not be escaped.  For example, \verb!\code{ "{" }! is legal.  The two
character sequence \verb!{}! may optionally follow zero-argument markup.
...
The zero argument macros may be followed by an opening and closing pair of 
braces (e.g. \verb!\R{}!).  
...
used to delimit arguments to markup macros, or in the special case of \verb!{}!
following zero argument macros.
...
Non-macro tags include \verb!TEXT!, \verb!RCODE!,
\verb!VERB!, \verb!COMMENT! or \verb!WHITESPACE!.
