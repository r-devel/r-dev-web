...
cd $SRC
RTEXE=`ls -1 rtools43-*.exe | head -1`
if [ "X$RTEXE" == X ] ; then
  echo "New Rtools43 installer not ready." >&2
  exit 1
fi
...
TBASE=gcc12_ucrt3_base_$TLVER.tar.zst
TFULL=gcc12_ucrt3_full_$TLVER.tar.zst
TCROSS=gcc12_ucrt3_cross_$TLVER.tar.zst
...
NTBASE=rtools43-toolchain-libs-base-$TLVER.tar.zst
NTFULL=rtools43-toolchain-libs-full-$TLVER.tar.zst
NTCROSS=rtools43-toolchain-libs-cross-$TLVER.tar.zst
...
svn cat https://svn.r-project.org/R-dev-web/trunk/WindowsBuilds/Rtools43/rtools.md | \
...
svn cat https://svn.r-project.org/R-dev-web/trunk/WindowsBuilds/Rtools43/news.md | \
