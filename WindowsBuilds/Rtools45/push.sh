...
cd $SRC
RTEXE=`ls -1 rtools45-*.exe | grep -v aarch64 | head -1`
if [ "X$RTEXE" == X ] ; then
  echo "New Rtools45 installer not ready." >&2
  exit 1
fi

RTEXEA=`ls -1 rtools45-*.exe | grep aarch64 | head -1`
if [ "X$RTEXEA" == X ] ; then
  echo "New Rtools45-aarch64 installer not ready." >&2
  exit 1
fi
...
TBASE=gcc14_ucrt3_base_$TLVER.tar.zst
TFULL=gcc14_ucrt3_full_$TLVER.tar.zst
TCROSS=gcc14_ucrt3_cross_$TLVER.tar.zst

RTVERA=`echo $RTEXEA | cut -d- -f3,4 | sed -e 's/\.exe//g'`
TLVERA=`echo $RTEXEA | cut -d- -f3`

TBASEA=llvm19_ucrt3_base_aarch64_$TLVERA.tar.zst
TFULLA=llvm19_ucrt3_full_aarch64_$TLVERA.tar.zst
TCROSSA=llvm19_ucrt3_cross_aarch64_$TLVERA.tar.zst
...
NTBASE=rtools45-toolchain-libs-base-$TLVER.tar.zst
NTFULL=rtools45-toolchain-libs-full-$TLVER.tar.zst
NTCROSS=rtools45-toolchain-libs-cross-$TLVER.tar.zst
NTCLB=`echo $TCLB | sed -e 's/Tcl-/tcltk-/g'`

NRTEXEA=`echo $RTEXEA | sed -e 's/\.exe/-unsigned.exe/g'`
NTBASEA=rtools45-toolchain-libs-base-aarch64-$TLVERA.tar.zst
NTFULLA=rtools45-toolchain-libs-full-aarch64-$TLVERA.tar.zst
NTCROSSA=rtools45-toolchain-libs-cross-aarch64-$TLVERA.tar.zst
...
svn cat https://svn.r-project.org/R-dev-web/trunk/WindowsBuilds/Rtools45/rtools.md | \
...
svn cat https://svn.r-project.org/R-dev-web/trunk/WindowsBuilds/Rtools45/news.md | \
