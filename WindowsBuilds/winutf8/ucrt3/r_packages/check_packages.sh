...
#    sed -E -i -e 's!'"$PCDIR"'/(CRAN|BIOC)/([^/]+)/\2\.Rcheck/(00check\.log|00install\.out)!https://www.r-project.org/nosvn/winutf8/ucrt3/\1/checks/gcc14-UCRT/packages/\2/\3.txt!g' $F
...
EOF
      ...
      if [ "$1" == REVDEPS ] ; then
        cat <<EOF | R --no-echo >>$RD/package/changes.txt
          old <- "`cygpath -m $UHOME/pkgcheck/CRAN/checks/gcc14-UCRT/export`"
