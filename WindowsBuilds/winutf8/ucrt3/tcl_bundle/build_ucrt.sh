...
if [ ! -r tcl8.6.17-src.tar.gz ] ; then
  wget https://prdownloads.sourceforge.net/tcl/tcl8.6.17-src.tar.gz
fi
if [ ! -r tk8.6.17-src.tar.gz ] ; then
  wget https://prdownloads.sourceforge.net/tcl/tk8.6.17-src.tar.gz
fi
if [ ! -r bwidget-1.10.1.zip ] ; then
  wget https://sourceforge.net/projects/tcllib/files/BWidget/1.10.1/bwidget-1.10.1.zip
fi
...
cd $BHOME/64bit
tar xfz $BHOME/tcl8.6.17-src.tar.gz
cd tcl8.6.17
...
cd $BINST/lib
unzip $BHOME/bwidget-1.10.1.zip
mv bwidget-1.10.1 BWidget

# build 64-bit Tk

cd $BHOME/64bit
tar xfz $BHOME/tk8.6.17-src.tar.gz
cd tk8.6.17
...
cp $BHOME/64bit/tcl8.6.17/pkgs/sqlite3.50.4/tclconfig/tcl.m4 tclconfig/tcl.m4
echo >> tclconfig/tcl.m4
aclocal -I tclconfig --force && autoconf --force
./configure --prefix=$BINST/lib --with-tcl=$BINST/lib --with-tclinclude=$BHOME/64bit/tcl8.6.17/generic --with-tk=$BINST/lib $CFARG --enable-threads --libdir=$BINST/lib --target=$TRIPLET --host=$TRIPLET 2>&1 | tee configure64.out
make -j 2>&1 | tee make64.out
make install 2>&1 | tee install64.out
