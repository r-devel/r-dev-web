...
\column{
  \begin{itemize}
   ...
   \item An embeddable extension language, interfaces to C, C++,
    FORTRAN, Java, Perl, Python, \ldots
  \end{itemize}
...
\column{
  ...
  (Austria), Thomas Lumley (USA), Martin Mächler (Switzerland), Guido
  Masarotto (Italy), Paul Murrell (NZ), Brian Ripley (UK), Duncan
  Temple Lang (USA), and Luke Tierney (USA).  In addition, there is a
  wealth of high-qulaity add-on packages provided by a very active
...
\column{
  ...
  \end{quote}
  and is mirrored to places all around the world \ldots\ one is
